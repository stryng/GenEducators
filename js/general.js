...
function adminfuncancel(filename) {
	ans=confirm('Are you sure, you want to cancel?');
	if(ans) {
		document.location.href="admin.php?file="+filename;
	} 
}
...
// developed by
// Amit Lad - 990-941-3547
// Parthiv Shah - 738-311-9597
// assing task by Hiren Mansuriya 
...
function getStateList(CountryID,selected)
	...
	function getCityList(StateID,selected)
	{
		if(!StateID)
		{
			...
			var link1="ajax/country_state_city.php?callfor=city&city="+StateID+"&selected="+selected;
		}
		$.ajax(
		{
			type: "POST",
...
function setvalue_of_revenue(obj,to_assign)
{
	var temp = (obj.value).split('cost_rev_value');
	if(temp[1] != null)
		document.getElementById(to_assign).value = temp[1];
	else
		document.getElementById(to_assign).value = "";
	$('#hours>option:eq(0)').attr('selected', true);
	$('#minut>option:eq(0)').attr('selected', true);
}
$(document).ready(function() {
...
{
	if(document.getElementById("userlistsupdate").innerHTML == "")
	{
		var link1="getUserNewList.php?new=true";
	}
	else
	{
		var link1="getUserNewList.php";
	}
	...
	$.ajax(
	{
		...
		success: function (data)
		{
			document.getElementById("userlistsupdate").innerHTML = document.getElementById("userlistsupdate").innerHTML+data
		}
	});
}