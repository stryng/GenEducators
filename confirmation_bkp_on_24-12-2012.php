<?
	...
	// Passkey that got from link
		...
		if(is_array($row) && sizeof($row)>0)
		{
				...
				{
					move_3_3($_GET['uid'],$passkey,$form);
					move_5_3($_GET['uid'],$passkey,$form);
					move_10_3($_GET['uid'],$passkey,$form);
				}
				$sql2="UPDATE userdetail SET active = 1 where UserId = '".$_GET['uid']."'";
				$row = $form->inser_qry($sql2);
				$myCustomField_1 = $_GET['uid'];
				$sql = "update userdetail set entry = 'prepaid' , entry_5_3 = 'prepaid' , entry_10_3 = 'prepaid',active = '1' where UserId = '".$myCustomField_1."'";
				$row = $form->inser_qry($sql);
						
				$sql = "insert into m_payment_fee(usertb_id, payment_val, payment_datetime, activedeactive, payment_done) values('".$myCustomField_1."','33','".date("Y-m-d H:i:s")."','active','YES')";
				$row = $form->inser_qry($sql);
				
				$sql = "insert into m_payment_fee_5_3(usertb_id, payment_val, payment_datetime, activedeactive, payment_done) values('".$myCustomField_1."','33','".date("Y-m-d H:i:s")."','active','YES')";
				$row = $form->inser_qry($sql);
				
				$sql = "insert into m_payment_fee_10_3(usertb_id, payment_val, payment_datetime, activedeactive, payment_done) values('".$myCustomField_1."','33','".date("Y-m-d H:i:s")."','active','YES')";
				$row = $form->inser_qry($sql);
...
function move_5_3($userids,$passkey,$form)
{
					...
					{
						print_r($rws);
						echo $msg = "User already registered, Please contact admin.";
						die;
					}
					else
					{
