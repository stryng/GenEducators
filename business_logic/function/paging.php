...
if (!isset($nstart)) {
	if ($num_totrec) { // if recs exists!!
		...
		for($i = 1;$i <= $num_pgs;$i++) {
			if ($start == $i) {
				...
				$var_link .= "<a ".$class." href=\"$var_self$var_file_url&nstart=1&start=$i$var_filter$var_sort_link$var_extra\">$i</a>";
			} 
		} 
		$var_link .="</span>";
		...
		if ($_REQUEST['start']!='') {
			...
			$var_link .= "<a tabindex=\"0\" class=\"next paginate_button\" id=\"DataTables_Table_0_next\"  href=\"$var_self$var_file_url&nstart=1&start=2$var_filter$var_sort_link$var_extra\">Next </a>";
			}
		} 
		$page_link = "";
	...
	if ($num_totrec) { // if recs exists!!
		...
		if ($var_flag != "0" and $var_flg != "2") {
			...
			$var_link .= "
				<a tabindex=\"0\" class=\"next paginate_button\" id=\"DataTables_Table_0_next\" href=\"$var_self$var_file_url&nstart=$num_nstart&start=$num_start$var_filter$var_sort_link$var_extra\"> Next</a>
